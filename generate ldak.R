gprob<-read.table("gl_doubleFiltered_variants.mpgl")
dim(gprob)
length(gprob[,1])
...
pcg<-prcomp(gprobMat,center=TRUE,scale=FALSE)
dim(pcg)
head(pcg)
library(MASS)
...
library("data.table")
library("readr")
gprob<-fread("tran_common_locinew.txt")
dim(gprob)
gprobMat<-gprob[,-1]
pcg<-prcomp(gprobMat,center=TRUE,scale=FALSE)
dim(pcg)
...
write.table(round(ldak2$posterior,5),file="ldak2new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak3$posterior,5),file="ldak3new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak4$posterior,5),file="ldak4new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak5$posterior,5),file="ldak5new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak6$posterior,5),file="ldak6new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak7$posterior,5),file="ldak7new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak8$posterior,5),file="ldak8new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak7$posterior,5),file="ldak9new.txt",quote=F,row.names=F,col.names=F)
write.table(round(ldak8$posterior,5),file="ldak10new.txt",quote=F,row.names=F,col.names=F)
